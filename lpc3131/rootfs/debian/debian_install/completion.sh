...
echo "#############################################################" >> $logfile_build
	...
	sed 's/root:x:0:0:root:\/root:\/bin\/bash/root::0:0:root:\/root:\/bin\/bash/g' "${output_dir}/mnt_debootstrap/etc/passwd" > tmp_file
	echo "$(cat tmp_file)" > "${output_dir}/mnt_debootstrap/etc/passwd" 
